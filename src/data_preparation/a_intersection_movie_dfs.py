...
def intersection_movie_dfs():
    # tmdb_movies_file = open('../../data/raw-data/tmdb-movies.json')
    tmdb_movies_file = requests.get(
        'https://bucket-recommender.s3.us-east-1.amazonaws.com/tmdb-movies.json',
    ...
    for tmdb_movie in tmdb_movies:
        try:
            c += 1
            kaggleId = kaggle_movies[kaggle_movies['title'] ==
                                     tmdb_movie['title']]['movieId'].values[0]
    ...
    with open(
            os.path.join(dirname,
                         '../../data/semi-treated-data/movies_kaggleId.json'),
            'w') as output_file:
