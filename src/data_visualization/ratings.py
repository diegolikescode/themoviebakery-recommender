...
for rating_val in list_ratings_raw:
    raw_list_values[rating_val] = ratings_raw_df[ratings_raw_df['rating'] == rating_val]['rating'].count()
...
for rating_val in list_ratings:
  list_values[rating_val] = ratings_full_df[ratings_full_df['rating'] == rating_val]['rating'].count()
...
ratings_smaller = pd.read_csv('../../data/data-for-analysis/ratings-smaller.csv')

list_ratings_smaller = np.sort(ratings_smaller['rating'].unique(), kind='quicksort')

list_values_smaller = {}
for rating_val in list_ratings_smaller:

  list_values_smaller[rating_val] = ratings_smaller[ratings_smaller['rating'] == rating_val]['rating'].count()
